...
const { sourceFormatter, loopStatusFormatter } = require('../../functions/formatter');
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addStringOption((option) =>
					option.setName('index').setDescription('Song index number').setRequired(true)
				)
		),
		...
		if (interaction.options.getSubcommand() === 'list') {
			const tracks = queue.tracks.map(
				(track, index) =>
					...
					} • ${sourceFormatter(queue.currentTrack.source)} • ${track.duration}`
			);
			...
			const embed = new EmbedBuilder()
				...
				.setDescription(
					...
					} • ${sourceFormatter(queue.currentTrack.source, queue.currentTrack.views)} • ${
				...
				.setFooter({
					text: `${queue.durationFormatted} • Loop status: ${loopStatusFormatter(
						queue.repeatMode
					)}`,
				});
			const msg = await interaction.editReply({ embeds: [embed] });
			...
			try {
				...
				const embed = new EmbedBuilder()
					...
					.setDescription(`Removed ${tracks[songIndex - 1]} from the queue`);
				const msg = await interaction.editReply({ embeds: [embed] });
